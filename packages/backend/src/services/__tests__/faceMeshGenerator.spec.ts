...
import type { FacialLandmark } from 'shared';
...
describe('FaceMeshGenerator', () => {
  ...
  describe('validateMeshQuality', () => {
    ...
    it('should detect invalid triangle indices', async () => {
      ...
      mesh.triangles.push({ vertices: [0, 1, 999] as [number, number, number] });
      
      const validation = await faceMeshGenerator.validateMeshQuality(mesh);
      
      expect(validation.isValid).toBe(false);
      expect(validation.errors.length).toBeGreaterThan(0);
      // Check if any error mentions invalid vertex index or internal error
      const hasRelevantError = validation.errors.some(error => 
        error.includes('Invalid vertex index') || error.includes('internal error')
      );
