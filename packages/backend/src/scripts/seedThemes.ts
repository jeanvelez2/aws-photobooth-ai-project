...
async function getCloudFrontUrl(bucketUrlParam?: string): Promise<string> {
  ...
  if (process.env.CLOUDFRONT_URL) return process.env.CLOUDFRONT_URL;
  
  try {
    const secretsClient = new SecretsManagerClient({ region: process.env.AWS_REGION || 'us-east-1' });
    const environment = process.env.NODE_ENV === 'production' ? 'prod' : 'dev';
    const secretName = `photobooth-config-${environment}`;
...
// Function to convert relative URLs to absolute CloudFront URLs
function makeAbsoluteUrl(relativeUrl: string, cloudFrontUrl: string): string {
  ...
  return `${cloudFrontUrl}/${cleanRelativeUrl}`;
}
  ...
  try {
    const themes = [
      {
        ...
        thumbnailUrl: 'themes/barbarian/barbarian-warrior-thumb.jpg', templateUrl: 'themes/barbarian/barbarian-template.jpg',
        variants: [
      ...
      },
      {
        ...
        thumbnailUrl: 'themes/greek/greek-hero-thumb.jpg', templateUrl: 'themes/greek/greek-template.jpg',
        variants: [
      ...
      },
      {
        ...
        thumbnailUrl: 'themes/mystic/mystic-wizard-thumb.jpg', templateUrl: 'themes/mystic/mystic-template.jpg',
        variants: [
      ...
      },
      {
        ...
        thumbnailUrl: 'themes/anime/anime-samurai-thumb.jpg', templateUrl: 'themes/anime/anime-template.jpg',
        variants: [
    ...
    for (const theme of themes) {
      ...
      const processedTheme = {
        ...theme,
        thumbnailUrl: makeAbsoluteUrl(theme.thumbnailUrl, CLOUDFRONT_URL),
        templateUrl: makeAbsoluteUrl(theme.templateUrl, CLOUDFRONT_URL),
        variants: theme.variants.map(variant => ({
          ...variant,
          thumbnailUrl: makeAbsoluteUrl(variant.thumbnailUrl, CLOUDFRONT_URL),
          templateUrl: makeAbsoluteUrl(variant.templateUrl, CLOUDFRONT_URL),
          blendingMask: makeAbsoluteUrl(variant.blendingMask, CLOUDFRONT_URL)
        }))
      };
