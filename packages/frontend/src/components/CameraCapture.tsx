...
export default function CameraCapture({ 
  ...
  // Get available camera devices (simple version)
  ...
  // Initialize camera on mount (only once, handles React StrictMode)
  useEffect(() => {
  ...
  }, [camera.stream, camera.isActive]); // Re-run when stream or active state changes

  // Cleanup on unmount - disabled for now to avoid React StrictMode issues
  // useEffect(() => {
  //   return () => {
  //     stopCamera();
  //   };
  ...
  const enableCamera = async () => {
    ...
    try {
      // Try to request permissions and get stream directly
      ...
      // Function to set video stream with retry logic
      ...
      // Try to set video stream immediately and with retries
      setVideoStream();

      shouldCleanupRef.current = true; // Mark that we should cleanup on unmount
  ...
  // Debug logging
  console.log('CameraCapture render:', { 
    isActive: camera.isActive, 
    error: camera.error, 
    hasStream: !!camera.stream,
    isLoading: ui.isLoading,
    shouldShowVideo: camera.isActive && !camera.error,
        ...
        {camera.isActive && !camera.error && camera.stream ? (
          <>
            ...
            <div className="absolute inset-0 pointer-events-none">
              ...
              {(ui.isLoading || isCapturing || uploadState.isUploading) && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                  <div className="text-white text-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white mx-auto mb-2"></div>
                    <p>
                      {isCapturing ? 'Capturing...' : 
                       uploadState.isUploading ? `Uploading... ${uploadState.progress?.percentage || 0}%` :
                       'Loading camera...'}
                    </p>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex justify-center space-x-4">
            ...
            <button
              ...
              disabled={isCapturing || ui.isLoading || uploadState.isUploading}
            ...
            {!autoUpload && state.app.currentPhoto && onPhotoUploaded && (
              <button
